...
export function activate(context: vscode.ExtensionContext) {
  ...
  FileType.allFileType.forEach((el) => {
    const cmd = vscode.commands.registerCommand(el.command, () => goToFile(el));
...
function onChangeTextEditor() {
  ...
  vscode.commands.executeCommand(
    "setContext",
    "nagular.showActionButton",
    uri &&
      (uri.includes(".component.") ||
        uri.includes(".directive.") ||
        uri.includes(".pipe.") ||
        uri.includes(".service.")),
  );
}
